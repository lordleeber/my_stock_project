...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch run evaluate.py for a range of months.")
...
def main() -> None:
    ...
    print(f"Batch evaluate: {start[0]}/{start[1]:02d} → {end[0]}/{end[1]:02d}  ({len(months)} months)")
    ...
    for year, month in months:
        ...
        if args.skip_existing:
            out_path = ROOT_DIR / "models_eps" / str(year) / month_s / "evaluate_by_fold.json"
            if out_path.exists():
