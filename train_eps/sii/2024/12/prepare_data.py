...
# 12/10 視角 -> anchor_eps=Q3EPS + 10月與11月營收，預測 Q4 EPS
FEATURES = [
    "anchor_eps",
    "ly_q4_eps",
    "q3_yoy_eps",
    ...
    "q3_ocf_ratio",
    "q3_re_ratio",
    "rev_yoy_m11_quantile",
    "rev_mom_m11_m10_quantile",
    "margin_momentum",
    "q3_roe",
    "q3_debt_ratio",
    "q3_non_op_ratio",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare v10_t4 dataset (12月視角) from DB.")
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    ...
    m10, m11 = f"{year}M10", f"{year}M11"
    ly_m10, ly_m11 = f"{year-1}M10", f"{year-1}M11"
    
    month_start, month_end = f"{year}-12-01", f"{year}-12-31"
    cutoff = f"{year}-12-10"
    ...
    mr = fetch_all_rows_api(api_base, "/raw/monthly-revenue", {"start_date": ly_m10, "end_date": m11})
    ...
    this_monthly = pd.DataFrame(columns=["symbol", "rev_m10", "rev_m11", "rev_m10_ly", "rev_m11_ly"])
    if not mr.empty:
        if "market" in mr.columns:
            mr = mr[mr["market"].astype(str).str.upper() == market.upper()].copy()
        mr2 = mr[mr["date"].isin([m10, m11, ly_m10, ly_m11])].copy()
        if not mr2.empty:
            pvt = mr2.pivot_table(index="symbol", columns="date", values="revenue_current", aggfunc="last").reset_index()
            this_monthly = pvt.rename(columns={m10: "rev_m10", m11: "rev_m11", ly_m10: "rev_m10_ly", ly_m11: "rev_m11_ly"})
            for c in ["rev_m10", "rev_m11", "rev_m10_ly", "rev_m11_ly"]:
                if c not in this_monthly.columns:
                    this_monthly[c] = np.nan
            this_monthly = this_monthly[["symbol", "rev_m10", "rev_m11", "rev_m10_ly", "rev_m11_ly"]]
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    this_monthly AS (
      ...
      WHERE date IN ('{m10}','{m11}','{ly_m10}','{ly_m11}')
      GROUP BY symbol
    ),
    eps_hist AS (
    ...
    SELECT {year} AS year, '{cutoff}' AS feature_cutoff_date, q3.*, q2.q2_margin,
           m.rev_m10,m.rev_m11,m.rev_m10_ly,m.rev_m11_ly,
...
def main() -> None:
    ...
    if args.data_source == "db":
        ...
        with engine.connect() as conn:
            ...
            for year in range(args.start_year, args.end_year + 1):
                print(f"fetching v10(12) data from db: year={year}, market={args.market}")
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v10(12) data from api: year={year}, market={args.market}")
    ...
    df["rev_yoy_m11"] = (df["rev_m11"] / df["rev_m11_ly"].replace(0, 1e-9)) - 1
    df["rev_mom_m11_m10"] = (df["rev_m11"] / df["rev_m10"].replace(0, 1e-9)) - 1
    ...
    print("v10(12) prepare_data completed")
