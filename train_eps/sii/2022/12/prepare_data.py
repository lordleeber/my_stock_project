...
MODEL_FEATURES = [
    ...
    # A/B test note (2022/12): "plus" version (keep M10 features + add M11 features)
    # performed better than "replace" version (M11-only replacement).
    ...
    # XBRL-enriched features (Q3 snapshot for December prediction)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare dataset (December view) from DB/API.")
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    ...
    this_monthly = pd.DataFrame(columns=["symbol", "rev_m9", "rev_m10", "rev_m11", "rev_m10_ly", "rev_m11_ly"])
    if not mr.empty:
        if "market" in mr.columns:
            mr = mr[mr["market"].astype(str).str.upper() == market.upper()].copy()
        mr2 = mr[mr["date"].isin([m9, m10, m11, ly_m10, ly_m11])].copy()
        if not mr2.empty:
            pvt = mr2.pivot_table(index="symbol", columns="date", values="revenue_current", aggfunc="last").reset_index()
            this_monthly = pvt.rename(columns={m9: "rev_m9", m10: "rev_m10", m11: "rev_m11", ly_m10: "rev_m10_ly", ly_m11: "rev_m11_ly"})
            for c in ["rev_m9", "rev_m10", "rev_m11", "rev_m10_ly", "rev_m11_ly"]:
                if c not in this_monthly.columns:
                    this_monthly[c] = np.nan
            this_monthly = this_monthly[["symbol", "rev_m9", "rev_m10", "rev_m11", "rev_m10_ly", "rev_m11_ly"]]
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    this_monthly AS (
      ...
      WHERE date IN ('{m9}','{m10}','{m11}','{ly_m10}','{ly_m11}')
      GROUP BY symbol
    ),
    eps_hist AS (
    ...
    SELECT {year} AS year, q3.*, q2.q2_margin, q2.q2_rev, q2.q2_ni,
           m.rev_m9,m.rev_m10,m.rev_m11,m.rev_m10_ly,m.rev_m11_ly,
