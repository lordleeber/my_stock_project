...
# 01/10 視角 -> anchor_eps=Q3EPS + 10,11,12月營收，預測 Q4 EPS
# 注意：01 月視角是在隔年的 1月10日 執行 (例如 year=2024 時，01月視角是在 2025-01-10)
# 因此 market snapshot 和 date cutoff 是使用 year+1。
FEATURES = [
    "anchor_eps",
    "ly_q4_eps",
    "q3_yoy_eps",
    ...
    "q3_ocf_ratio",
    "q3_re_ratio",
    "rev_yoy_m12_quantile",
    "rev_mom_m12_m11_quantile",
    "margin_momentum",
    "q3_roe",
    "q3_debt_ratio",
    "q3_non_op_ratio",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare v10_t4 dataset (01月視角) from DB.")
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    ...
    m11, m12 = f"{year}M11", f"{year}M12"
    ly_m11, ly_m12 = f"{year-1}M11", f"{year-1}M12"
    
    # 01月視角是在 year+1 發生的
    next_year = year + 1
    month_start, month_end = f"{next_year}-01-01", f"{next_year}-01-31"
    cutoff = f"{next_year}-01-10"
    ...
    mr = fetch_all_rows_api(api_base, "/raw/monthly-revenue", {"start_date": ly_m11, "end_date": m12})
    ...
    this_monthly = pd.DataFrame(columns=["symbol", "rev_m11", "rev_m12", "rev_m11_ly", "rev_m12_ly"])
    if not mr.empty:
        if "market" in mr.columns:
            mr = mr[mr["market"].astype(str).str.upper() == market.upper()].copy()
        mr2 = mr[mr["date"].isin([m11, m12, ly_m11, ly_m12])].copy()
        if not mr2.empty:
            pvt = mr2.pivot_table(index="symbol", columns="date", values="revenue_current", aggfunc="last").reset_index()
            this_monthly = pvt.rename(columns={m11: "rev_m11", m12: "rev_m12", ly_m11: "rev_m11_ly", ly_m12: "rev_m12_ly"})
            for c in ["rev_m11", "rev_m12", "rev_m11_ly", "rev_m12_ly"]:
                if c not in this_monthly.columns:
                    this_monthly[c] = np.nan
            this_monthly = this_monthly[["symbol", "rev_m11", "rev_m12", "rev_m11_ly", "rev_m12_ly"]]
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    this_monthly AS (
      SELECT symbol,
             MAX(CASE WHEN date='{m11}' THEN revenue_current END) AS rev_m11,
             MAX(CASE WHEN date='{m12}' THEN revenue_current END) AS rev_m12,
             MAX(CASE WHEN date='{ly_m11}' THEN revenue_current END) AS rev_m11_ly,
             MAX(CASE WHEN date='{ly_m12}' THEN revenue_current END) AS rev_m12_ly
      FROM monthly_revenue
      WHERE date IN ('{m11}','{m12}','{ly_m11}','{ly_m12}')
      GROUP BY symbol
    ),
    eps_hist AS (
    ...
    SELECT {year} AS year, '{cutoff}' AS feature_cutoff_date, q3.*, q2.q2_margin,
           m.rev_m11,m.rev_m12,m.rev_m11_ly,m.rev_m12_ly,
...
def main() -> None:
    ...
    if args.data_source == "db":
        ...
        with engine.connect() as conn:
            ...
            for year in range(args.start_year, args.end_year + 1):
                print(f"fetching v10(01) data from db: year={year}, market={args.market}")
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v10(01) data from api: year={year}, market={args.market}")
    ...
    df["rev_yoy_m12"] = (df["rev_m12"] / df["rev_m12_ly"].replace(0, 1e-9)) - 1
    df["rev_mom_m12_m11"] = (df["rev_m12"] / df["rev_m11"].replace(0, 1e-9)) - 1
    ...
    # 01 月視角同樣僅能使用已公告到 Q3 的資訊 (Q4尚未公告)
    df["ttm_eps_official"] = (
    ...
    print("v10(01) prepare_data completed")
