...
# 03/31 視角 -> anchor_eps=Q4EPS (年報出爐) + 01,02月合併營收，預測本年度 Q1 EPS
FEATURES = [
    "anchor_eps",
    "ly_q1_eps",
    "q4_yoy_eps",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare v10_t4 dataset (03月視角) from DB.")
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    # 預測當年度 Q1
    # anchor 為 year-1 的 Q4 (於03月底全數公布)
    q1 = f"{year}Q1"
    ly_q1 = f"{year-1}Q1"
    ...
    m01, m02 = f"{year}M01", f"{year}M02"
    ly_m01, ly_m02 = f"{year-1}M01", f"{year-1}M02"
    
    month_start, month_end = f"{year}-03-01", f"{year}-03-31"
    cutoff = f"{year}-03-31"
    ...
    mr = fetch_all_rows_api(api_base, "/raw/monthly-revenue", {"start_date": ly_m01, "end_date": m02})
    ...
    this_monthly = pd.DataFrame(columns=["symbol", "rev_m01", "rev_m02", "rev_m01_ly", "rev_m02_ly"])
    if not mr.empty:
        if "market" in mr.columns:
            mr = mr[mr["market"].astype(str).str.upper() == market.upper()].copy()
        mr2 = mr[mr["date"].isin([m01, m02, ly_m01, ly_m02])].copy()
        if not mr2.empty:
            pvt = mr2.pivot_table(index="symbol", columns="date", values="revenue_current", aggfunc="last").reset_index()
            this_monthly = pvt.rename(columns={m01: "rev_m01", m02: "rev_m02", ly_m01: "rev_m01_ly", ly_m02: "rev_m02_ly"})
            for c in ["rev_m01", "rev_m02", "rev_m01_ly", "rev_m02_ly"]:
                if c not in this_monthly.columns:
                    this_monthly[c] = np.nan
            this_monthly = this_monthly[["symbol", "rev_m01", "rev_m02", "rev_m01_ly", "rev_m02_ly"]]
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    this_monthly AS (
      ...
      WHERE date IN ('{m01}','{m02}','{ly_m01}','{ly_m02}')
      GROUP BY symbol
    ),
    eps_hist AS (
    ...
    SELECT {year} AS year, '{cutoff}' AS feature_cutoff_date, q4.*, q3.q3_margin,
           m.rev_m01,m.rev_m02,m.rev_m01_ly,m.rev_m02_ly,
...
def main() -> None:
    ...
    if args.data_source == "db":
        ...
        with engine.connect() as conn:
            ...
            for year in range(args.start_year, args.end_year + 1):
                print(f"fetching v10(03) data from db: year={year}, market={args.market}")
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v10(03) data from api: year={year}, market={args.market}")
    ...
    # 重點：對抗農曆新年，計算 M1與M2合併營收 YoY
    df["rev_m01_m02"] = df["rev_m01"].fillna(0) + df["rev_m02"].fillna(0)
    df["rev_m01_m02_ly"] = df["rev_m01_ly"].fillna(0) + df["rev_m02_ly"].fillna(0)
    df["rev_yoy_m01_m02"] = (df["rev_m01_m02"] / df["rev_m01_m02_ly"].replace(0, 1e-9)) - 1

    add_industry_zscore(df, "rev_yoy_m01_m02", "rev_yoy_m01_m02_z")
    add_cross_section_quantile(df, "rev_yoy_m01_m02_z", "rev_yoy_m01_m02_quantile")
    ...
    # 03 月視角 Q4財報剛出
    df["ttm_eps_official"] = (
        df["q1_eps"].fillna(0) if "q1_eps" in df.columns else df["ly_q1_eps"].fillna(0)  # ly_q1 is effectively q1 if we are viewing from next year q1
        + df["q2_eps"].fillna(0)
        + df["q3_eps"].fillna(0)
    ...
    print("v10(03) prepare_data completed")
