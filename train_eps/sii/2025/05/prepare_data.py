...
# v10: 5/14 閬? -> anchor_eps=Q1EPS + 4???塚??怠?瘥??Ｘ平璅???
FEATURES = [
    "anchor_eps",
    "ly_q2_eps",
    "q1_yoy_eps",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare v10 dataset (5??閫? from DB or API.")
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    ...
    if not qr.empty:
        ...
        eps_hist = p.rename(columns={
            q2: "target_eps", lyq3: "ly_q3_eps", lyq2: "ly_q2_eps", lyq1: "ly_q1_eps",
...
def main() -> None:
    ...
    # anchor_eps = q1_eps嚗?/6/7??閫芋?絞銝?賢?嚗?
    ...
    # 5 ??閫??賭蝙?典歇?砍???Q1 ??閮??踹??撟?Q2嚗?砍?嚗葆?仿?瘣拇?
