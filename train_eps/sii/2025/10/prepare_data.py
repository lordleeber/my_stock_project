...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare v10_t3 dataset for analysis_codex from DB.")
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    ...
    ly_m7, ly_m8, ly_m9 = f"{year-1}M07", f"{year-1}M08", f"{year-1}M09"
    ...
    qr = fetch_all_rows_api(api_base, "/raw/quarterly-reports", {"start_date": lyq3, "end_date": q3, "market": market})
    mr = fetch_all_rows_api(api_base, "/raw/monthly-revenue", {"start_date": ly_m7, "end_date": m9})
    ...
    this_monthly = pd.DataFrame(columns=["symbol", "rev_m7", "rev_m8", "rev_m9", "rev_m7_ly", "rev_m8_ly", "rev_m9_ly"])
    if not mr.empty:
        if "market" in mr.columns:
            mr = mr[mr["market"].astype(str).str.upper() == market.upper()].copy()
        mr2 = mr[mr["date"].isin([m7, m8, m9, ly_m7, ly_m8, ly_m9])].copy()
        if not mr2.empty:
            pvt = mr2.pivot_table(index="symbol", columns="date", values="revenue_current", aggfunc="last").reset_index()
            this_monthly = pvt.rename(columns={m7: "rev_m7", m8: "rev_m8", m9: "rev_m9", ly_m7: "rev_m7_ly", ly_m8: "rev_m8_ly", ly_m9: "rev_m9_ly"})
            for c in ["rev_m7", "rev_m8", "rev_m9", "rev_m7_ly", "rev_m8_ly", "rev_m9_ly"]:
                if c not in this_monthly.columns:
                    this_monthly[c] = np.nan
            this_monthly = this_monthly[["symbol", "rev_m7", "rev_m8", "rev_m9", "rev_m7_ly", "rev_m8_ly", "rev_m9_ly"]]

    eps_hist = pd.DataFrame(columns=["symbol", "target_eps", "ly_q3_eps", "prev_q4_eps", "q1_eps", "q2_eps_official"])
    if not qr.empty:
        qr2 = qr[qr["date"].isin([q3, lyq3, p4, q1, q2])].copy()
        p = qr2.pivot_table(index="symbol", columns="date", values="eps_q", aggfunc="last").reset_index()
        eps_hist = p.rename(columns={q3: "target_eps", lyq3: "ly_q3_eps", p4: "prev_q4_eps", q1: "q1_eps", q2: "q2_eps_official"})
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    this_monthly AS (
      ...
      WHERE date IN ('{m7}','{m8}','{m9}','{ly_m7}','{ly_m8}','{ly_m9}')
      GROUP BY symbol
    ),
    eps_hist AS (
    ...
    SELECT {year} AS year, '{cutoff}' AS feature_cutoff_date, q2.*, q1.q1_margin,
           m.rev_m7,m.rev_m8,m.rev_m9,m.rev_m7_ly,m.rev_m8_ly,m.rev_m9_ly,
...
def main() -> None:
    ...
    if args.data_source == "db":
        ...
        with engine.connect() as conn:
            ...
            for year in range(args.start_year, args.end_year + 1):
                print(f"fetching v10_t3 data from db: year={year}, market={args.market}")
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v10_t3 data from api: year={year}, market={args.market}")
    ...
    df["rev_yoy_m9"] = (df["rev_m9"] / df["rev_m9_ly"].replace(0, 1e-9)) - 1
    df["rev_mom_m8_m7"] = (df["rev_m8"] / df["rev_m7"].replace(0, 1e-9)) - 1
    df["rev_mom_m9_m8"] = (df["rev_m9"] / df["rev_m8"].replace(0, 1e-9)) - 1

    add_industry_zscore(df, "rev_yoy_m7", "rev_yoy_m7_z")
    add_industry_zscore(df, "rev_yoy_m8", "rev_yoy_m8_z")
    add_industry_zscore(df, "rev_yoy_m9", "rev_yoy_m9_z")
    add_industry_zscore(df, "rev_mom_m8_m7", "rev_mom_m8_m7_z")
    add_industry_zscore(df, "rev_mom_m9_m8", "rev_mom_m9_m8_z")
    ...
    add_cross_section_quantile(df, "rev_yoy_m9_z", "rev_yoy_m9_quantile")
    add_cross_section_quantile(df, "rev_mom_m8_m7_z", "rev_mom_m8_m7_quantile")
    add_cross_section_quantile(df, "rev_mom_m9_m8_z", "rev_mom_m9_m8_quantile")
    ...
    print("v10_t3 prepare_data completed")
