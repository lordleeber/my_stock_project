...
# 02/10 或 02/15 視角 -> anchor_eps=Q3EPS + 01月營收，預測隔年 Q1 EPS
FEATURES = [
    "anchor_eps",
    "ly_q1_eps",
    "q3_yoy_eps",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Prepare v10_t4 dataset (02月視角) from DB.")
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    # 預測當年度 Q1 (例如 year=2021 時預測 2021Q1)
    # anchor 為 year-1 的 Q3
    q1 = f"{year}Q1"
    ly_q1 = f"{year-1}Q1"
    q3_anchor = f"{year-1}Q3"
    ly_q3_anchor = f"{year-2}Q3"
    q2_for_mom = f"{year-1}Q2"
    prev_q4_eps = f"{year-2}Q4"
    
    m01 = f"{year}M01"
    ly_m01 = f"{year-1}M01"
    
    month_start, month_end = f"{year}-02-01", f"{year}-02-28"
    cutoff = f"{year}-02-15"

    inc_q3 = fetch_all_rows_api(api_base, "/raw/income-statements", {"start_date": q3_anchor, "end_date": q3_anchor, "market": market})
    inc_q2 = fetch_all_rows_api(api_base, "/raw/income-statements", {"start_date": q2_for_mom, "end_date": q2_for_mom, "market": market})
    bs_q3 = fetch_all_rows_api(api_base, "/raw/balance-sheets", {"start_date": q3_anchor, "end_date": q3_anchor, "market": market})
    cf_q3 = fetch_all_rows_api(api_base, "/raw/cash-flows", {"start_date": q3_anchor, "end_date": q3_anchor, "market": market})
    qr = fetch_all_rows_api(api_base, "/raw/quarterly-reports", {"start_date": ly_q3_anchor, "end_date": q1, "market": market})
    mr = fetch_all_rows_api(api_base, "/raw/monthly-revenue", {"start_date": ly_m01, "end_date": m01})
    ...
    this_monthly = pd.DataFrame(columns=["symbol", "rev_m01", "rev_m01_ly"])
    if not mr.empty:
        if "market" in mr.columns:
            mr = mr[mr["market"].astype(str).str.upper() == market.upper()].copy()
        mr2 = mr[mr["date"].isin([m01, ly_m01])].copy()
        if not mr2.empty:
            pvt = mr2.pivot_table(index="symbol", columns="date", values="revenue_current", aggfunc="last").reset_index()
            this_monthly = pvt.rename(columns={m01: "rev_m01", ly_m01: "rev_m01_ly"})
            for c in ["rev_m01", "rev_m01_ly"]:
                if c not in this_monthly.columns:
                    this_monthly[c] = np.nan
            this_monthly = this_monthly[["symbol", "rev_m01", "rev_m01_ly"]]

    eps_hist = pd.DataFrame(columns=["symbol", "target_eps", "ly_q1_eps", "ly_q3_eps", "prev_q4_eps", "q2_eps", "q3_eps_official"])
    if not qr.empty:
        qr2 = qr[qr["date"].isin([q1, ly_q1, ly_q3_anchor, prev_q4_eps, q2_for_mom, q3_anchor])].copy()
        p = qr2.pivot_table(index="symbol", columns="date", values="eps_q", aggfunc="last").reset_index()
        eps_hist = p.rename(columns={q1: "target_eps", ly_q1: "ly_q1_eps", ly_q3_anchor: "ly_q3_eps", prev_q4_eps: "prev_q4_eps", q2_for_mom: "q2_eps", q3_anchor: "q3_eps_official"})
        for c in ["target_eps", "ly_q1_eps", "ly_q3_eps", "prev_q4_eps", "q2_eps", "q3_eps_official"]:
            if c not in eps_hist.columns:
                eps_hist[c] = np.nan
        eps_hist = eps_hist[["symbol", "target_eps", "ly_q1_eps", "ly_q3_eps", "prev_q4_eps", "q2_eps", "q3_eps_official"]]
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    WITH q3_data AS (
      ...
      WHERE i.date='{q3_anchor}' AND i.market='{market}'
    ),
    q2_data AS (
      SELECT symbol, net_income_q/NULLIF(revenue_q,0) AS q2_margin
      FROM income_statement WHERE date='{q2_for_mom}' AND market='{market}'
    ),
    this_monthly AS (
      SELECT symbol,
    ...
    eps_hist AS (
      SELECT qr.symbol, qr.eps_q AS target_eps,
             ...
             (SELECT eps_q FROM quarterly_reports WHERE symbol=qr.symbol AND date='{ly_q3_anchor}' AND market='{market}') AS ly_q3_eps,
             (SELECT eps_q FROM quarterly_reports WHERE symbol=qr.symbol AND date='{prev_q4_eps}' AND market='{market}') AS prev_q4_eps,
             (SELECT eps_q FROM quarterly_reports WHERE symbol=qr.symbol AND date='{q2_for_mom}' AND market='{market}') AS q2_eps,
             (SELECT eps_q FROM quarterly_reports WHERE symbol=qr.symbol AND date='{q3_anchor}' AND market='{market}') AS q3_eps_official
    ...
    SELECT {year} AS year, '{cutoff}' AS feature_cutoff_date, q3.*, q2.q2_margin,
           m.rev_m01,m.rev_m01_ly,
           e.target_eps,e.ly_q1_eps,e.ly_q3_eps,e.prev_q4_eps,e.q2_eps,e.q3_eps_official,
...
def main() -> None:
    ...
    if args.data_source == "db":
        ...
        with engine.connect() as conn:
            ...
            for year in range(args.start_year, args.end_year + 1):
                print(f"fetching v10(02) data from db: year={year}, market={args.market}")
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v10(02) data from api: year={year}, market={args.market}")
    ...
    df["rev_yoy_m01"] = (df["rev_m01"] / df["rev_m01_ly"].replace(0, 1e-9)) - 1

    add_industry_zscore(df, "rev_yoy_m01", "rev_yoy_m01_z")
    add_cross_section_quantile(df, "rev_yoy_m01_z", "rev_yoy_m01_quantile")

    # Q1 seasonality
    df["ly_seasonality"] = (df["ly_q1_eps"] / df["ly_q3_eps"].replace(0, 1e-9)).clip(-5, 5)
    ...
    print("v10(02) prepare_data completed")
