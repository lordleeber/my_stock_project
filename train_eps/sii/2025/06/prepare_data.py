...
# v10: 6/10 視角 -> Q1 + 4/5月營收，anchor = q1_eps
FEATURES = [
    "anchor_eps",
    "ly_q3_eps",
    "q1_margin",
...
def fetch_one_year_api(api_base: str, year: int, market: str) -> pd.DataFrame:
    ...
    if not mr.empty:
        ...
        mr2 = mr[mr["date"].isin([m4, m5, ly_m4, ly_m5])].copy()
        if not mr2.empty:
            pivot = mr2.pivot_table(index="symbol", columns="date", values="revenue_current", aggfunc="last").reset_index()
            this_monthly = pivot.rename(columns={m4: "rev_m4", ly_m4: "rev_m4_ly", m5: "rev_m5", ly_m5: "rev_m5_ly"})
            for c in ["rev_m4", "rev_m4_ly", "rev_m5", "rev_m5_ly"]:
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    this_monthly AS (
      ...
      FROM monthly_revenue WHERE date IN ('{m4}','{m5}','{ly_m4}','{ly_m5}') GROUP BY symbol
    ),
    eps_hist AS (
    ...
    if args.data_source == "db":
        ...
        with engine.connect() as conn:
            ...
            for year in range(args.start_year, args.end_year + 1):
                print(f"fetching v10(06) data from db: year={year}, market={args.market}")
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v10(06) data from api: year={year}, market={args.market}")
