...
This is the trap: each row's `year/month` is **the month the rotation happens**, but `realized_net_pnl` belongs to the **previous month's cohort** (the one being sold). Two time axes in one row.

| Column | What it points at |
|---|---|
| `year`, `month` | **The rotation event** — which month is doing the buy-new / sell-old swap |
...
| `cohort_year`, `cohort_month` | **The cohort being settled** — the one whose PnL appears in `realized_net_pnl` (= `year/month - 1` calendar-wise; NaN in the first iteration when portfolio was empty) |
...
### Concrete example
Row `year=2026, month=04, entry_date=2026-04-13, cohort_year=2026, cohort_month=03, cohort_entry_date=2026-03-11, realized_net_pnl=335596.49`:
- The **March cohort** entered 2026-03-11 and was sold at 2026-04-10 open for net +335,596.
- The April cohort entered 2026-04-13; its PnL will only appear later in the row for `year=2026, month=05` (when it's sold at 2026-05-15 open).

### Looking up a cohort's PnL
`df[(df.cohort_year == 2026) & (df.cohort_month == "03")]['realized_net_pnl']`

### Why this is the trap
A naive `groupby('year', 'month').sum('realized_net_pnl')` will attribute March's PnL to April — wrong. Always group by `cohort_year/cohort_month` when summarizing per-cohort performance.

## Still-Open Positions
The final iteration's new cohort is left in the portfolio without being settled. `rolling_summary.json` reports `still_open_count` and `rolling_trades.csv` has rows with `exit_date=NaN`. To realize them, extend `--end_month` by one (which requires next month's `candidates_scored.csv` to exist).
...
- Walk-forward rule: month M uses the latest selection model with cutoff < M. Run `strategies/step5_score_and_publish.py` for month M before backtesting through M.
