...
def predict_with_uncertainty(model: CatBoostRegressor, x_data: pd.DataFrame, train_center: float) -> tuple[np.ndarray, np.ndarray]:
    # CatBoost 這裡使用「距離訓練預測中位數」作為不確定度 proxy
...
def main() -> None:
    ...
    for test_year in years[1:]:
        ...
        pred_eps_cat = test_df["q2_eps"].to_numpy(dtype=float) + pred_delta

        pred_eps_q2 = test_df["q2_eps"].to_numpy(dtype=float)
        pred_eps_med = np.full(len(test_df), float(train_df[TARGET].median()), dtype=float)

        for model_name, pred in [("cat_delta", pred_eps_cat), ("baseline_q2_eps", pred_eps_q2), ("baseline_train_median", pred_eps_med)]:
        ...
        tmp["y_true"] = y_true
        tmp["pred_cat_delta"] = pred_eps_cat
        ...
        val = build_valuation_daily_frame(test_df, pred_eps_cat, f"cat_v9_t3_{args.feature_transform}", "analysis_catboost/v9/t3/results/predictions.csv")
