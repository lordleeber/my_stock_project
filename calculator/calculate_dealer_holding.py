...
# Cumulative SUM: each new row's dealer_held_shares = previous cumulative total
# (from existing rows) + cumulative SUM of new dealer_net deltas.
# Window starts from the per-(market, symbol) baseline pulled from last_state.
INSERT_SQL = """
INSERT INTO {table} (
    date, market, symbol, name, issued_shares,
    dealer_held_shares, dealer_held_ratio,
    pced_file, pced_row, pced_col
)
WITH last_state AS (
    SELECT DISTINCT ON (market, symbol)
        market, symbol, dealer_held_shares AS last_shares
    FROM {table}
...
new_daily AS (
    SELECT
        ...
        MAX(ii.name) AS name,
        SUM(COALESCE(ii.dealer_net, 0)) AS dealer_net
...
new_cum AS (
    SELECT
        ...
        COALESCE(ls.last_shares, 0) + SUM(nd.dealer_net) OVER (
...
SELECT
    ...
    fh.issued_shares,
    nc.dealer_held_shares,
    CASE WHEN fh.issued_shares > 0
         THEN ROUND((nc.dealer_held_shares / fh.issued_shares * 100)::numeric, 4)
         ELSE NULL
    END AS dealer_held_ratio,
    'calculated_dealer_holding'::text, 0::bigint, 'x'::text
FROM new_cum nc
...
def _create_table_if_missing(conn):
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS {TABLE} (
            ...
            dealer_held_shares DOUBLE PRECISION,
            dealer_held_ratio DOUBLE PRECISION,
            pced_file TEXT,
            pced_row BIGINT,
            pced_col TEXT,
...
def main():
    force_full = parse_force_full("Compute dealer_holding incrementally.")
    try:
