...
def run():
    ...
    df_preds = pd.read_sql(
        "SELECT symbol, target_quarter, predict_eps FROM eps_predictions", engine
    )
    ...
    df_eps["latest_3_sum"] = df_eps.groupby("symbol")["eps_q"].transform(
    ...
    df_eps = df_eps.merge(
        df_preds,
        left_on=["symbol", "next_q"],
        right_on=["symbol", "target_quarter"],
        how="left",
    )
    df_eps["ttm_eps_forward"] = df_eps["latest_3_sum"] + df_eps["predict_eps"].fillna(0)
    df_eps.loc[df_eps["predict_eps"].isna(), "ttm_eps_forward"] = df_eps[
        "ttm_eps_official"
    ]
    ...
    df_combined = df_combined[
        (df_combined["ttm_eps_official"] > 0) & (df_combined["ttm_eps_forward"] > 0)
    ]

    df_combined["pe_calculated"] = (
        df_combined["close"] / df_combined["ttm_eps_official"]
    ).round(2)
    df_combined["pe_forward"] = (
        df_combined["close"] / df_combined["ttm_eps_forward"]
    ).round(2)
    df_combined["predict_target_price"] = (
        df_combined["ttm_eps_forward"] * df_combined["pe_official"]
    ).round(2)
    df_combined["upside_pct"] = (
        (df_combined["predict_target_price"] / df_combined["close"] - 1) * 100
    ).round(2)
    df_combined["roe_official"] = (
        df_combined["ttm_eps_official"] / df_combined["nav_per_share"] * 100
    ).round(2)
    df_combined["roe_forward"] = (
        df_combined["ttm_eps_forward"] / df_combined["nav_per_share"] * 100
    ).round(2)
