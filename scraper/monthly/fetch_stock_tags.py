...
def get_all_categories(session):
    ...
    try:
        ...
        matches = re.findall(
            r'href="(/z/zh/zha/zh\d{2}\.djhtm\?a=([^"&]+))"[^>]*>([^<]+)</a>', text
        )

        categories = []
        for href, cat_id, name in matches:
            categories.append(
                {"name": name.strip(), "id": cat_id, "url": f"{BASE_URL}{href}"}
            )
...
def _parse_stocks_from_tables(html_text):
    ...
    for df in tables:
        ...
        if not any(
            "代號" in c or "股票代號" in c or "Code" in c or "股票名稱" in c
            for c in cols
        ):
            continue
...
def main():
    ...
    parser.add_argument(
        "--max-categories", type=int, default=0, help="Limit categories for quick test."
    )
