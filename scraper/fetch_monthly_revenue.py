    ...
    try:
        ...
        for df in dfs:
            ...
            for i in range(min(5, len(df))):
                ...
                row_values_norm = [v.replace('\n', '').replace(' ', '') for v in row_values_raw]
                
                # Check for key columns in this row
                if any("公司代號" in v for v in row_values_norm) and any("當月營收" in v for v in row_values_norm):
                    header_row_idx = i
                    break
            ...
            clean_df = clean_df[clean_df['公司代號'].astype(str).str.match(r'^\d{4}[A-Za-z0-9]*$')]
        ...
        if e.response.status_code == 404:
            print(f"❌ Report not found (404). Data might not be available yet for {year_roc}/{month}.")
        else:
...
def process_and_save(year, month):
    ...
    numeric_cols = ['revenue', 'revenue_last_month', 'revenue_last_year', 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--year", type=int, help="Year (AD), e.g., 2023", required=False)
    ...
    if args.year and args.month:
        ...
        print(f"No date specified. Defaulting to last month: {target_year}/{target_month}")
