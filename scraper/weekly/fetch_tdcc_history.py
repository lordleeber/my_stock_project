...
class TDCCScraper:
    ...
    def initialize(self):
        ...
        try:
            ...
            if date_select:
                self.available_dates = [
                    option["value"] for option in date_select.find_all("option")
                ]
                logger.info(
                    f"Found {len(self.available_dates)} available dates. Latest: {self.available_dates[0]}"
                )
            else:
    ...
    def scrape_stock(self, stock_no, date_str):
        ...
        payload = {
            ...
            "SYNCHRONIZER_URI": self.current_uri
            if self.current_uri
            else "/portal/zh/smWeb/qryStock",
            "method": "submit",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch TDCC Shareholding Dispersion History"
    )
    ...
    parser.add_argument(
        ...
        "-f",
        type=str,
        help="File containing list of stock IDs (one per line)",
    )
    parser.add_argument(
        "--date",
        "-d",
        type=str,
        help="Date to fetch (YYYYMMDD) or file containing list of dates",
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--list-dates",
        action="store_true",
        help="List all available dates from TDCC website",
    )
    parser.add_argument(
    ...
    for date_str in date_list:
        ...
        if date_str not in scraper.available_dates:
            logger.warning(
                f"Date {date_str} might not be available. Available dates (top 5): {scraper.available_dates[:5]}"
            )
        ...
        for idx, stock_id in enumerate(stock_list):
            ...
            if os.path.exists(output_path):
                if FORCE_REPROCESS:
                    logger.info(
                        f"[{idx + 1}/{len(stock_list)}] {stock_id} exists for {date_str}, reprocessing due to FORCE_REPROCESS=1."
                    )
                else:
                    logger.info(
                        f"[{idx + 1}/{len(stock_list)}] {stock_id} already exists for {date_str}. Skipping."
                    )
                    continue

            logger.info(
                f"[{idx + 1}/{len(stock_list)}] Fetching {stock_id} for {date_str}..."
            )
            ...
            if df is not None and not df.empty:
                ...
                logger.warning(
                    f"Failed to fetch or empty data for {stock_id} on {date_str}"
                )
