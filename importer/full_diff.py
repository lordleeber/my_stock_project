...
def export_table_to_df(engine, table_name):
    ...
    with engine.connect() as conn:
        ...
        try:
            ...
            data_dict = {
                col: [row[i] for row in rows] for i, col in enumerate(col_names)
            }
...
def compare_dataframes(df_csv, df_db, table_name):
    ...
    # 創建複合鍵
    df_csv = df_csv.with_columns(pl.concat_str(key_cols, separator="|").alias("_key"))
    df_db = df_db.with_columns(pl.concat_str(key_cols, separator="|").alias("_key"))
