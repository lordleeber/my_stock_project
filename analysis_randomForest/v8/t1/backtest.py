...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest for analysis/v8_t1")
...
def evaluate_metrics(df_eval: pd.DataFrame, y_true: np.ndarray, y_pred: np.ndarray, eps_floor: float) -> dict:
    ...
    true_forward_pe[valid_price & eps_true_ok] = close[valid_price & eps_true_ok] / y_true[valid_price & eps_true_ok]
    pred_forward_pe[valid_price & eps_pred_ok] = close[valid_price & eps_pred_ok] / y_pred[valid_price & eps_pred_ok]
...
def main() -> None:
    ...
    for test_year in years[1:]:
        ...
        for model_name, pred_eps in [("rf_delta", pred_eps_rf), ("baseline_q2_eps", pred_eps_baseline_q2), ("baseline_train_median", pred_eps_baseline_median)]:
            m = evaluate_metrics(test_df, y_true, pred_eps, args.eps_floor)
            fold_rows.append({"version": "v8_t1", "protocol": "expanding_by_year", "fold": f"year_{test_year}", "model": model_name, **m, "n_train": len(train_df), "n_test": len(test_df)})

        keep_cols = [c for c in ["year", "symbol", "name", "feature_cutoff_date", "q3_date", "q3_close", "q3_volume", "pe_current", "ttm_eps_official"] if c in test_df.columns]
        tmp = test_df[keep_cols].copy()
        tmp["y_true"] = y_true
        tmp["pred_rf_delta"] = pred_eps_rf
        tmp["pred_delta_std"] = pred_delta_std
        tmp["confidence_threshold"] = confidence_threshold
        tmp["pred_baseline_q2_eps"] = pred_eps_baseline_q2
        tmp["pred_baseline_train_median"] = pred_eps_baseline_median
        tmp["fold"] = f"year_{test_year}"
        pred_rows.append(tmp)

        val_df = build_valuation_daily_frame(test_df, pred_eps_rf, "v8_t1", "analysis_codex/v8/t1/results/predictions.csv")
    ...
    if not valuation_df.empty:
        ...
        volume_ok = (valuation_df["volume"] / 1000.0) >= float(args.min_volume_lots)
        valuation_df = valuation_df[ttm_ok & volume_ok].copy()

    for frame, nd in [(fold_df, 2), (pred_df, 2), (valuation_df, 4)]:
    ...
    print("v8_t1 backtest completed")
    print(f"- {fold_path}")
