...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare v8_t1 dataset for analysis_codex from DB.")
...
def main() -> None:
    ...
    with engine.connect() as conn:
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v8_t1 data: year={year}, market={args.market}")
    ...
    df["ttm_eps_official"] = (
        df["prev_q4_eps"].fillna(0) + df["q1_eps"].fillna(0) + df["q2_eps_official"].fillna(0) + df[TARGET].fillna(0)
    )
    ...
    print("v8_t1 prepare_data completed")
