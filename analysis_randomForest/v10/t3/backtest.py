...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest for analysis/v10_t3")
...
def main() -> None:
    ...
    if not val_df.empty:
        ...
        # 只保留 forward 不低於 official 的樣本（剔除 forward 變差）
        forward_not_worse = val_df["ttm_eps_forward"] >= val_df["ttm_eps_official"]
        val_df = val_df[ttm_ok & vol_ok & forward_not_worse].copy()
    ...
    print("v10_t3 backtest completed")
    print(f"- {fold_path}")
