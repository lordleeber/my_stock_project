...
def main() -> None:
    ...
    for test_year in years[1:]:
        ...
        for model_name, pred in [("rf_delta", pred_eps_rf), ("baseline_q2_eps", pred_eps_q2), ("baseline_train_median", pred_eps_med)]:
            ...
            row = {"version": "v10_t1", "protocol": "expanding_by_year", "fold": f"year_{test_year}", "model": model_name, **m, "n_train": len(train_df), "n_test": len(test_df)}
        ...
        val = build_valuation_daily_frame(
            ...
            pred_eps_high,
            "v10_t1",
            "analysis_codex/v10/t1/results/predictions.csv",
        )
