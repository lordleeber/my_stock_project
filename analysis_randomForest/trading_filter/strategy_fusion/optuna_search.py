...
from analysis_randomForest.trading_filter.strategy_fusion.backtest import run_fusion_once
...
OUT_ALL = OUT_DIR / "optuna_results_all.csv"
OUT_TOP20 = OUT_DIR / "optuna_results_top20.csv"
OUT_BEST = OUT_DIR / "optuna_best_config.json"
OUT_SUMMARY = OUT_DIR / "optuna_study_summary.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optuna search for fusion strategy")
...
def main() -> None:
    ...
    def objective(trial):
        score_threshold = float(trial.suggest_float("score_threshold", 0.25, 0.60, step=0.01))
        max_picks = int(trial.suggest_categorical("max_picks", [20, 30, 40, 50, 60, 80, 100]))
        max_per_industry = int(trial.suggest_categorical("max_per_industry", [3, 5, 8, 10, 12]))
        max_position_amount = float(
            trial.suggest_categorical("max_position_amount", [100000.0, 150000.0, 200000.0, 300000.0])
        )
