...
from analysis_randomForest.trading_filter.strategyF.grid_search import run_one_combo
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="StrategyF Optuna parameter search")
...
def main() -> None:
    ...
    def objective(trial):
        rs_mode = trial.suggest_categorical("rs_mode", ["upside_only", "upside_plus_delta", "composite"])
        top_quantile = float(trial.suggest_categorical("top_quantile", [0.50, 0.60, 0.70, 0.80]))
        min_upside_ratio = float(trial.suggest_categorical("min_upside_ratio", [1.03, 1.05, 1.08]))
        tp_pct = float(trial.suggest_categorical("tp_pct", [0.10, 0.12, 0.14]))
    ...
    print("strategyF optuna search ready/done")
    print(f"- all: {OUT_ALL}")
