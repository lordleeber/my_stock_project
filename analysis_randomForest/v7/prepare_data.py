...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare v7 dataset for analysis_codex from DB.")
...
def main() -> None:
    ...
    with engine.connect() as conn:
        ...
        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v7 data: year={year}, market={args.market}")
    ...
    # 三個月營收波動（變異係數）作為 regime 切分特徵
    rev_mean = (df["rev_m7"] + df["rev_m8"] + df["rev_m9"]) / 3.0
    rev_std = df[["rev_m7", "rev_m8", "rev_m9"]].std(axis=1)
    df["rev_volatility"] = (rev_std / rev_mean.replace(0, 1e-9)).clip(0, 10)
    ...
    print("v7 prepare_data completed")
