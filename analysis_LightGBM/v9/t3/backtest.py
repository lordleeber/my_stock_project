...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest for analysis/v9_t3")
...
def predict_with_uncertainty(model: LGBMRegressor, x_data: pd.DataFrame, train_center: float) -> tuple[np.ndarray, np.ndarray]:
    # LightGBM 沒有 RF 那種可直接取每棵樹預測分散度，這裡用距離訓練預測中位數作為不確定度 proxy
    pred = model.predict(x_data)
    uncertainty = np.abs(pred - train_center)
...
def main() -> None:
    ...
    for test_year in years[1:]:
        ...
        train_pred = model.predict(train_df[use_features])
        train_center = float(np.median(train_pred))
        pred_delta_raw, pred_delta_std = predict_with_uncertainty(model, test_df[use_features], train_center)
        _, pred_delta_train_std = predict_with_uncertainty(model, train_df[use_features], train_center)
        ...
        pred_eps_lgb = test_df["q2_eps"].to_numpy(dtype=float) + pred_delta

        pred_eps_q2 = test_df["q2_eps"].to_numpy(dtype=float)
        pred_eps_med = np.full(len(test_df), float(train_df[TARGET].median()), dtype=float)

        for model_name, pred in [("lgb_delta", pred_eps_lgb), ("baseline_q2_eps", pred_eps_q2), ("baseline_train_median", pred_eps_med)]:
        ...
        tmp["y_true"] = y_true
        tmp["pred_lgb_delta"] = pred_eps_lgb
        ...
        val = build_valuation_daily_frame(test_df, pred_eps_lgb, f"lgb_v9_t3_{args.feature_transform}", "analysis_LightGBM/v9/t3/results/predictions.csv")
    ...
    print("v9_t3 backtest completed")
    print(f"- {fold_path}")
