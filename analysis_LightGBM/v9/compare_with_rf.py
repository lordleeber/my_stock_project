...
def main() -> None:
    ...
    for slice_name in ["t1", "t2", "t3"]:
        ...
        lgb_pred = pd.read_csv(BASE_DIR / slice_name / "results" / "predictions.csv")

        key_cols = ["fold", "symbol"]
        common = rf_pred[key_cols].drop_duplicates().merge(
            lgb_pred[key_cols].drop_duplicates(),
            on=key_cols,
            how="inner",
        )
        ...
        rows.append(
            {
                "slice": slice_name,
                "model": "rf_delta",
                **metrics(y_true, rf_eval["pred_rf_delta"].to_numpy(float)),
            }
        )
        rows.append(
            {
                "slice": slice_name,
                "model": "lgb_delta",
                **metrics(y_true, lgb_eval["pred_lgb_delta"].to_numpy(float)),
            }
        )
        rows.append(
            {
                ...
                **metrics(y_true, rf_eval["pred_baseline_q2_eps"].to_numpy(float)),
            }
        )
    ...
    out_path = BASE_DIR / "rf_vs_lgb_compare.csv"
