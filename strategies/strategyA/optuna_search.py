...
from strategies.strategyA.grid_search import build_config, run_one_combo
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="StrategyA Optuna parameter search")
...
def main() -> None:
    ...
    def objective(trial):
        tp_pct = float(trial.suggest_float("tp_pct", 0.01, 0.15, step=0.01))
        sl_pct = float(trial.suggest_float("sl_pct", 0.01, 0.10, step=0.01))
        max_hold_days = int(trial.suggest_categorical("max_hold_days", [5, 7, 10, 12, 15, 20, 30]))

        cfg = build_config(tp_pct=tp_pct, sl_pct=sl_pct, max_hold_days=max_hold_days)
        row = run_one_combo(
        ...
        # 鈭斗?蝑?????湧??砍?嚗?甇支??潮?瑼餅???蝵?        entered_count = int(row["entered_count"])
    ...
    print("strategyA optuna search ready/done")
    print(f"- all: {OUT_ALL}")
