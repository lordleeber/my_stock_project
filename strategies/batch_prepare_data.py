...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch run prepare_data.py for a range of months.")
...
def main() -> None:
    ...
    print(f"Batch prepare_data: {start[0]}/{start[1]:02d} → {end[0]}/{end[1]:02d}  ({len(months)} months)")
    ...
    for year, month in months:
        ...
        if args.skip_existing:
            out_path = ROOT_DIR / "strategies" / "output" / str(year) / month_s / "dataset_strategy.csv"
            if out_path.exists():
