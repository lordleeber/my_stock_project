...
系統內建強大的回測引擎，專門驗證「量能爆發」策略的有效性，並提供多種進階濾網以優化勝率。
...
### 進階濾網與優化
*   **紅 K 濾網 (Red Candle Filter)**: 
    *   `Enabled`: 僅在爆量當天為「紅 K 棒」(Close > Open) 時才進場。
    *   **效果**: 實測顯示可過濾掉 40% 的無效交易，顯著提升 ROI 與勝率。
*   **重複加碼 (Pyramiding)**:
    *   `Enabled`: 若持倉期間再次出現訊號，則繼續買入 (獲利最大化)。
    *   `Disabled`: 若已有持倉，忽略新訊號直到賣出 (嚴格控制曝險)。

### 實測績效參考 (2025/05)
*測試條件：固定金額 10 萬、單一部位 (不加碼)、持有 3 天*

| 策略設定 | 總交易次數 | 總獲利 (NT$) | ROI | 勝率 | 備註 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **基礎策略** | 289 次 | $111,137 | 0.38% | 43.25% | 交易過於頻繁 |
| **加入紅 K 濾網** | **170 次** | **$196,769** | **1.16%** | **47.65%** | **勝率與效率大幅提升** |
