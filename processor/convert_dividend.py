...
def process_file(csv_file):
    try:
        ...
        col_indices = {}
        for col in ["date", "symbol", "name", "close_before", "ref_price", "rights_dividend_value", "type"]:
        ...
        for row_idx in range(header_idx + 1, len(rows)):
            row = rows[row_idx]
            if len(row) <= max(col_indices.values(), default=0): continue
            
            symbol = row[col_indices["symbol"]].strip()
            # 嚴格 4 碼過濾
            if not re.match(r"^\d{4}$", symbol): continue
            ...
            data = {
                ...
                "rights_dividend_value": clean_numeric(row[col_indices["rights_dividend_value"]]),
...
def main():
    ...
    for year_dir in sorted(raw_path.glob("20*")):
        ...
        if df is not None:
            ...
            rename_map = {"src_file": "pced_file", "src_row": "pced_row", "src_col": "pced_col"}
