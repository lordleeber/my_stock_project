...
class DataQualityCheckerBase(ABC):
    ...
    def __init__(self, date_str):
        ...
        # Detailed column audit output control:
        # - AUDIT_SYMBOL: target symbol to print (default: 2330)
        # - AUDIT_PRINT_COLUMNS: 1 to enable detailed per-column output (default: 0)
        self.audit_symbol = os.getenv("AUDIT_SYMBOL", "2330").strip()
        self.audit_print_columns = os.getenv("AUDIT_PRINT_COLUMNS", "0") == "1"
    ...
    def _verify_lineage(self, df, market):
        ...
        for idx, row in df.iterrows():
            ...
            for col_idx, (col_name, src_idx_str) in enumerate(
                ...
                match = self._compare_values(processed_val, raw_val, col_name)
                if trace_row:
                    self._print_trace_column(
                        col_name=col_name,
                        src_idx_str=src_idx_str,
    ...
    def _should_trace_row(self, row):
        """Whether to print detailed per-column audit output for this row."""
        if not self.audit_print_columns:
            return False
        if not self.audit_symbol:
            return False
        symbol_val = row.get("symbol", "")
        symbol_clean = clean_value_for_comparison(symbol_val)
        return symbol_clean == self.audit_symbol

    def _print_trace_row_header(self, label, idx, row, src_file, src_row):
    ...
    def _print_trace_column(
        ...
        print(
            f"   [{status}] {col_name} <- src_col[{src_idx_str}] "
            f"processed='{processed_val}' raw='{raw_val}'{extra}"
        )
