...
def validate_and_map_columns(
    df_columns: list, use_mapping: dict, is_english: bool, file_path: str
) -> tuple:
    """
...
def process_monthly_revenue():
    ...
    if not start_env or not end_env:
        ...
        print(
            "Example: START_DATE=20240101 END_DATE=20240131 python convert_monthly.py"
        )
        sys.exit(1)
    ...
    for dir_path in all_dirs:
        ...
        for file_path in csv_files:
            try:
                ...
                df = pd.read_csv(
                    file_path, header=header_row, encoding="utf-8", thousands=","
                )
                ...
                if "market" in df.columns:
                    ...
                    market = (
                        "SII"
                        if "sii" in file_path.lower()
                        else "OTC"
                        if "otc" in file_path.lower()
                        else "UNKNOWN"
                    )
                ...
                if DEBUG:
                    print(
                        f"  ✓ Parsed {file_path}: {len(df)} rows, src_col={new_df['src_col'].iloc[0] if len(new_df) > 0 else 'N/A'}"
                    )
        ...
        try:
            ...
            checker.check()
            print(
                f"✅ Monthly revenue data quality check passed for {year_str}M{month_str}"
            )
