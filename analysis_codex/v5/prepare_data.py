...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare v5 dataset for analysis_codex from DB.")
...
def fetch_one_year(conn, year: int, market: str) -> pd.DataFrame:
    ...
    JOIN target_q3 t ON q2.symbol = t.symbol
...
def main() -> None:
    ...
    with engine.connect() as conn:
        # 避免資料庫端平行查詢吃掉共享記憶體，降低 OOM 風險
        conn.execute(text("SET max_parallel_workers_per_gather = 0"))

        for year in range(args.start_year, args.end_year + 1):
            print(f"fetching v5 data: year={year}, market={args.market}")
    ...
    print("v5 prepare_data completed")
