...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch run strategies/cache_daily_quotes.py by month range.")
...
def main() -> None:
    ...
    cache_script = (Path(__file__).resolve().parent.parent / "strategies" / "cache_daily_quotes.py").resolve()
    ...
    for year, month in iter_months(args.start_year, args.start_month, args.end_year, args.end_month):
        ...
        print(f"\n=== [{year}/{month_s}] cache_daily_quotes ===")
        print("Command:", " ".join(cmd))
        result = subprocess.run(cmd)
        if result.returncode != 0:
            raise RuntimeError(f"cache_daily_quotes failed at {year}/{month_s} (code={result.returncode})")
        total += 1

    print(f"\nbatch_cache_daily_quotes done: {total} month(s)")
