...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch run strategies/prepare_data.py by month range.")
...
def main() -> None:
    ...
    prepare_script = (Path(__file__).resolve().parent.parent / "strategies" / "prepare_data.py").resolve()
    ...
    for year, month in iter_months(args.start_year, args.start_month, args.end_year, args.end_month):
        ...
        print(f"\n=== [{year}/{month_s}] prepare_data ===")
        print("Command:", " ".join(cmd))
        result = subprocess.run(cmd)
        if result.returncode != 0:
            raise RuntimeError(f"prepare_data failed at {year}/{month_s} (code={result.returncode})")
        total += 1

    print(f"\nbatch_prepare_data done: {total} month(s)")
