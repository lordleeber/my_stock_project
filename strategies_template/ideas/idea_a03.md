...
## 最新回測結果（2026-03-07 回寫）
- `2022-08 ~ 2022-12`
  - Main `return_pct = -5.1797%`
  - Baseline `return_pct = -5.6137%`
  - 解讀：有小幅相對優勢，但仍然虧損，尚不足以證明策略穩健
- `2023-01 ~ 2023-12`
  - Main `return_pct = 5.7920%`
  - Baseline `return_pct = -0.6016%`
  - 解讀：這版策略在 2023 明顯成功，代表少量高 conviction 架構不是錯方向
- `2024-01 ~ 2024-12`
  - Main `gross_pnl = 53,602.19`
  - Main `total_cost = 52,266.07`
  - Main `net_pnl = 1,336.12`
  - Main `sold_count = 76`
  - 解讀：2024 的核心問題仍是 cost drag。毛利幾乎完全被成本吃掉，表示交易仍偏多、單筆 edge 不夠厚
- `2025-01 ~ 2025-10`
  - Main `return_pct = 2.8229%`
  - Baseline `return_pct = 7.1260%`
  - 解讀：雖然是正報酬，但明顯落後 baseline。這代表問題不只在成本，而是多頭或強勢年份的收益捕捉不夠

## 目前判斷
- 這版策略不能算失敗，因為 `2023` 顯著優於 baseline。
- 但也不能視為完成，因為：
  - `2024` 幾乎沒有把毛利留下來
  - `2025` 雖然賺錢，但遠落後 baseline
- 因此此版目前定位應調整為：
  - `v3` 已證明高 conviction 方向可行
  - 但還需要一個下一版，去修正 `2024` 的成本拖累與 `2025` 的上漲年落後問題

## 下一輪修正方向
- 不能只再把交易數壓得更低，否則可能進一步犧牲 `2025` 的收益捕捉。
- 下一版應優先檢查：
  - entry 門檻是否過嚴
  - Top `8 ~ 12` 是否在某些 regime 下過少
  - 是否錯過高趨勢但波動稍大的強勢股
- optimize 不應只看成本壓制，還要加入對 baseline 相對表現的觀察，避免出現：
  - 成本變小了
  - 但主策略在強勢年明顯少賺很多
